                         ...
                         Chassis_Mit_Output_t *output)
{
    ...
    const float base_current_a = output->position_current_a +
        ...
        config->speed_integral_limit_a > 0.0f)
    {
        const float integral_candidate_a = mit_limit(
            ...
            config->speed_integral_limit_a);
        const float candidate_current_a = base_current_a + integral_candidate_a;
        const uint8_t drives_out_of_saturation =
            (candidate_current_a > config->current_limit_a && speed_error_rpm < 0.0f) ||
            (candidate_current_a < -config->current_limit_a && speed_error_rpm > 0.0f);
        if (fabsf(candidate_current_a) <= config->current_limit_a ||
    ...
    output->raw_current_a = mit_limit(base_current_a +
